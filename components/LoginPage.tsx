            ...
            <div className="w-full max-w-md bg-white rounded-xl shadow-2xl p-8 space-y-8 animate-fade-in">
                ...
                <form className="space-y-5" onSubmit={handleLogin}>
                    <div>
                        ...
                        <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="w-full px-4 py-3 border rounded-lg focus:ring-2 ring-trimais-blue outline-none transition-all" />
                    </div>
                    <div>
                        <label className="block text-xs font-bold text-gray-400 uppercase mb-1">Senha de Acesso</label>
                        <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full px-4 py-3 border rounded-lg focus:ring-2 ring-trimais-blue outline-none transition-all" />
                    </div>
